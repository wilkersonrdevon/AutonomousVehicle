...
gyro_pub = rospy.Publisher("sensor_msgs/gyro", Imu, queue_size=50)
while True:
    h = Header()
    h.stamp = rospy.Time.now()
    h.frame_id = "gyro"
    Imu().angular_velocity.x, Imu().angular_velocity.y, Imu().angular_velocity.z = SENSOR.gyro
    gyro_pub.publish(h, Imu())
